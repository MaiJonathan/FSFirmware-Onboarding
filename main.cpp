...
int main()
{
   while(true) 
   {
      ...
      double brakePos = 0.5 * sensor3V - 0.125;
      ...
      {
         if(fabs(pos1 - pos2) / ((pos1 + pos2) / 2.0) > 0.10 || !expectedRange0 || !expectedRange1 ) //Check if positions differ by more than 10%  or APPs are not in their expected voltage range
         {
